...
public class Palavra {
	...
	/**
	 * Constante que indica que o "_" deve ser mostrado ao usuário enquanto ele
	 * não descobre a letra
	 */
	...
	/**
	 * Retorna a palavra. Se o jogador ainda não descobriu determinada letra, o
	 * {@link #CARACTERE_SECRETO} é mostrado no lugar dela.
	 * 
	...
	public String toString() {
		...
		for (int i = 0; i < letrasArray.length; i++) {
			...
			// Variável que armazena os caracteres da palavra sorteada.
			char c = letrasArray[i];

			/*
			 * Checa se o caractere está presente na lista de caracteres
			 * encontrados
			 */
			if (caracteresEncontrados.contains(c)) {

				/*
				 * Se está, significa que o usuário já identificou o caractere,
				 * portanto ele pode ser mostrado
				 */
				sb.append(c);
			} else {

				// Caso contrário, o "_" é exibido no lugar
		...
		// Converte o StringBuffer para string e o retorna.
	...
	public boolean possuiLetra(char letra) {
		...
		if (palavraOriginal.indexOf(letra) > -1) {
			...
			/*
			 * Antes de retornar, adiciona a letra na lista de letras
			 * encontradas
			 */
	...
	public boolean acertouPalavra() {
		...
		/*
		 * Adiciona todos os caracteres da palavra no conjunto. Se o caractere
		 * for repetido, ele não será adicionado.
		 */
		for (int i = 0; i < letrasArray.length; i++) {
		...
		/*
		 * Se o conjunto com os caracteres da palavra tiver os mesmos elementos
		 * do conjunto de caracteres encontrados pelo jogador, significa que ele
		 * já acetou todas as letras da palavra
		 */
