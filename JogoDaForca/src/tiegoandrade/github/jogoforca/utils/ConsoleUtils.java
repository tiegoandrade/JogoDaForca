...
import tiegoandrade.github.jogoforca.CaractereInvalidoException;
import tiegoandrade.github.jogoforca.JogoDaForcaException;
...
public class ConsoleUtils {
	...
	public static char lerLetra() throws JogoDaForcaException {
		...
		try {
			...
			if (linha.length() > 1) {
				throw new CaractereInvalidoException(
						"Apenas uma letra deve ser digitada");
			}
			...
			if (!Character.isLetter(letra)) {
				throw new CaractereInvalidoException(
						"Apenas letras devem ser digitadas");
			}

			return letra;

		} catch (IOException e) {
			throw new JogoDaForcaException(
					"Problema ao ler caractere do teclado", e);
		}
	}
}
